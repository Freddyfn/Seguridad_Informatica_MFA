...
\begin{tikzpicture}[
    ...
    client/.style={rectangle, rounded corners, draw=primary, fill=primary!10, very thick, text centered, text width=3cm, minimum height=1.5cm},
    server/.style={rectangle, draw=secondary, fill=secondary!10, very thick, text centered, text width=3cm, minimum height=1.5cm},
    database/.style={cylinder, draw=black, fill=gray!20, aspect=0.3, shape border rotate=90, text centered, text width=2.5cm, minimum height=2cm},
    mobile/.style={rectangle, rounded corners=0.2cm, draw=black, fill=white, thick, text centered, text width=2cm, minimum height=3cm},
    line/.style={draw, -Latex, thick},
    arrowtext/.style={font=\footnotesize, sloped, anchor=south}
]

% Nodes
\node (browser) [client] {\textbf{Navegador Web} \\ (React SPA)};
\node (mobile) [mobile, left=of browser] {\textbf{Dispositivo Móvil}\\ (App TOTP: Authy/Google Auth)};
\node (api) [server, below=3cm of browser] {\textbf{Servidor API Node.js} \\ (Express + otplib + JWT)};
\node (db) [database, right=of api] {\textbf{SQLite DB} \\ Users, Products, Orders};

% Encapsulation boxes
\begin{scope}[on background layer]
    \node [fill=blue!5, rounded corners, fit=(browser) (mobile), draw=blue!30, thick, inner sep=0.5cm] (clientzone) {};
    \node [anchor=north west, font=\bfseries\color{blue!60}] at (clientzone.north west) {Capa de Cliente};

    \node [fill=orange!5, rounded corners, fit=(api) (db), draw=orange!30, thick, inner sep=0.5cm] (serverzone) {};
    \node [anchor=north west, font=\bfseries\color{orange!60}] at (serverzone.north west) {Capa de Servidor (Backend Segura)};
\end{scope}

% Paths
\draw [line] (browser) to[bend left=15] node[arrowtext] {1. POST /login (Credenciales)} (api);
\draw [line] (api) to[bend left=15] node[arrowtext, below] {2. Retorna estado 2FA REQUIRED} (browser);

\draw [line, dashed, red] (mobile) -- node[arrowtext, rotate=90, anchor=south, color=red] {Escanea QR / Semilla} (browser);
\draw [line] (browser) to[bend left=45] node[arrowtext] {3. Input Código TOTP (6 dígitos)} (mobile);
\draw [line] (browser) -- node[arrowtext] {4. POST /login-verify (Código)} (api);

\draw [line] (api) to[bend left=15] node[arrowtext] {Query / Comparar Hashes} (db);
\draw [line] (db) to[bend left=15] node[arrowtext, below] {Datos Usuario / Secret} (api);

\draw [line, green!60!black] (api) to[out=135, in=225] node[arrowtext] {5. Emisión de JWT Válido} (browser);
